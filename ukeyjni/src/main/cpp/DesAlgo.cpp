/************************************************************************
*
*  Module name         : DesAlgo.cpp
*
*  Module description  :
*     DES encryption algorithm.
*     Large sections adapted from the 1977 public-domain program by
*     Jim Gillogly.
*
...
************************************************************************/
static void    DES(unsigned char *key, unsigned char *data, int doEncrypt);
...
/**
 * diversify key from main key
 */
void divsfKeyFun(unsigned char *mainKey,unsigned char *gen, unsigned char *sessionKey)
{
		unsigned char i = 0;
	memcpy(sessionKey, gen, 8);	

	DES(mainKey, sessionKey, 1);
	DES(mainKey+8, sessionKey, 0);
	DES(mainKey, sessionKey, 1);

	for( i=0 ; i < 8 ; i++ )
	{
		*(sessionKey + 8 + i) = *(gen + i) ^ 0xFF;
	}
	
	DES(mainKey, sessionKey+8, 1);
	DES(mainKey+8, sessionKey+8, 0);
	DES(mainKey, sessionKey+8, 1);
 ...
 */
void getSessionMacFun(unsigned char *sessionKey, unsigned char *input,
					int length, unsigned char *mac)
{
	unsigned long offset = 0;
	unsigned char i = 0;
	unsigned long srcLen = length;
	...
	while (srcLen >= 8)
	{
		for (i = 0; i < 8; i++) 
		{
			icv[i] ^= input[offset + i]; 
		}

		// DES cbc
		DES(sessionKey, icv, 1);
...
/************************************************************************
*  Function name   : DES
*  Description     : Perform DES encryption/decryption.
*                  :
*  Parameters      : key         - DES key.
*                  : data        - Data to be encrypted/decrypted.
*                  : doEncrypt   - 1 for encryption, 0 for descryption.
*  Returns         : -
*  Author          : Richard Shen
* ----------------------------------------------------------------------
*  Date     By       Description
